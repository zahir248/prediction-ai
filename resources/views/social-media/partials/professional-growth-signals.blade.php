...
<div class="professional-growth-container" style="margin-bottom: 32px; padding: 24px; background: white; border-radius: 12px; border: 1px solid #e2e8f0;">
    ...
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 16px;">
        <h3 style="font-size: 18px; font-weight: 600; color: #1e293b; margin: 0; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif;">Professional Growth Signals</h3>
        @if($confidence)
    ...
    <div style="display: grid; gap: 24px;">
        @foreach($indicators as $key => $indicator)
            @php
                ...
                // For PDF: Generate SVG for slider
                if ($isPdfExport) {
                    $sliderWidth = 400;
                    $sliderHeight = 60;
                    $trackHeight = 12;
                    $handleSize = 18;
                    $trackY = 30;
                    $handleX = ($sliderValue / 100) * $sliderWidth;
                    $handleY = $trackY;
                    
                    $sliderSvg = '<' . '?xml version="1.0" encoding="UTF-8"?>';
                    $sliderSvg .= '<svg xmlns="http://www.w3.org/2000/svg" width="' . $sliderWidth . '" height="' . $sliderHeight . '">';
                    
                    // Background track
                    $sliderSvg .= sprintf(
                        '<rect x="0" y="%s" width="%s" height="%s" rx="8" fill="#f1f5f9"/>',
                        $trackY - ($trackHeight / 2), $sliderWidth, $trackHeight
                    );
                    
                    // Progress track with solid color matching handle
                    $sliderSvg .= sprintf(
                        '<rect x="0" y="%s" width="%s" height="%s" rx="8" fill="%s"/>',
                        $trackY - ($trackHeight / 2), $handleX, $trackHeight, htmlspecialchars($handleColor)
                    );
                    
                    // Handle
                    $sliderSvg .= sprintf(
                        '<circle cx="%s" cy="%s" r="%s" fill="white" stroke="%s" stroke-width="3"/>',
                        $handleX, $handleY, $handleSize / 2, htmlspecialchars($handleColor)
                    );
                    ...
                    $sliderSvg .= '</svg>';
                    $sliderSvgDataUri = 'data:image/svg+xml;base64,' . base64_encode($sliderSvg);
                }
            @endphp
            <div style="padding: 20px; background: #f8fafc; border-radius: 8px; border: 1px solid #e2e8f0;">
                <h4 style="font-size: 16px; font-weight: 600; color: #1e293b; margin: 0 0 32px 0; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif;">
                    {{ $indicator['label'] }}
                </h4>
                ...
                <div style="margin-bottom: 12px;">
                    ...
                    @else
                        ...
                        <div style="position: relative; height: 12px; background: linear-gradient(to right, #f1f5f9 0%, #e2e8f0 100%); border-radius: 8px; overflow: visible; box-shadow: inset 0 2px 4px rgba(0,0,0,0.06);">
                            <!-- Slider track with solid color matching handle -->
