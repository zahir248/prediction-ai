...
<div style="margin-bottom: 32px; padding: 24px; background: white; border-radius: 12px; border: 1px solid #e2e8f0;">
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 16px;">
        <h3 style="font-size: 18px; font-weight: 600; color: #1e293b; margin: 0;">Political Communication Style</h3>
        @if($confidence)
    ...
    @if(isset($data['rhetoric_analysis']) && is_string($data['rhetoric_analysis']))
        <div style="margin-bottom: 24px; padding: 16px; background: #f8fafc; border-radius: 8px; border-left: 4px solid #667eea;">
    ...
    @php
        ...
        if ($isPdfExport) {
            ...
            $svg .= sprintf(
                '<polygon points="%s" fill="rgba(99, 102, 241, 0.2)" stroke="#6366f1" stroke-width="2"/>',
            ...
            foreach ($points as $index => $point) {
                ...
                // Point
                $svg .= sprintf(
                    '<circle cx="%s" cy="%s" r="6" fill="#4f46e5" stroke="white" stroke-width="2"/>',
                    ...
                    @foreach($points as $index => $point)
                        <g class="radar-point" data-dimension="{{ $point['key'] }}" style="cursor: pointer;">
                            <circle cx="{{ round($point['x'], 2) }}" cy="{{ round($point['y'], 2) }}" 
                                    r="6" fill="#4f46e5" stroke="white" stroke-width="2"/>
                            <circle cx="{{ round($point['x'], 2) }}" cy="{{ round($point['y'], 2) }}" 
    ...
    <!-- Tooltip Element -->
    <div id="political-communication-tooltip" style="position: fixed; background: #1f2937; color: white; padding: 12px 16px; border-radius: 8px; font-size: 13px; font-weight: 400; white-space: pre-line; width: 280px; z-index: 10000; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1); line-height: 1.6; pointer-events: none; opacity: 0; transition: opacity 0.2s; text-align: left; max-width: 280px;"></div>
    
    <script>
    document.addEventListener('DOMContentLoaded', function() {
        const tooltip = document.getElementById('political-communication-tooltip');
    ...
    @if(isset($data['overall_assessment']) || isset($data['communication_strengths']))
