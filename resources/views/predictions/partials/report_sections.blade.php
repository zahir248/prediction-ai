            ...
            <div style="margin-bottom: 32px;">
                <h2 style="font-size: 16px; font-weight: 600; color: #374151; margin-bottom: 16px; padding-bottom: 8px; border-bottom: 1px solid #e5e7eb;">{{ $__ui('Analysis Status', 'Status Analisis') }}</h2>
                <div style="display: flex; align-items: center; gap: 16px; flex-wrap: wrap;">
                    <span style="padding: 6px 12px; border-radius: 6px; font-size: 12px; font-weight: 600; 
                        ...
                        @endif">
                        {{ $__statusLabel($prediction->status) }}
                    </span>
                    <span style="color: #64748b; font-size: 13px;">
                        {{ $__ui('Period:', 'Tempoh:') }} {{ $__horizonLabel($prediction->prediction_horizon) }}
                    </span>
                    @php
                        $reportLang = $prediction->input_data['report_language'] ?? null;
                    @endphp
                    @if($reportLang === 'en' || $reportLang === 'ms')
                    <span style="color: #64748b; font-size: 13px;">
                        · {{ $__ui('Report language:', 'Bahasa laporan:') }} {{ $reportLang === 'ms' ? 'Bahasa Melayu' : 'English' }}
                    </span>
                    @endif
                </div>
            </div>
            ...
            <div style="margin-bottom: 32px;">
                <h2 style="font-size: 16px; font-weight: 600; color: #374151; margin-bottom: 16px; padding-bottom: 8px; border-bottom: 1px solid #e5e7eb;">{{ $__ui('Issue Details', 'Butiran Isu') }}</h2>
            ...
            <div style="margin-bottom: 32px;">
                <h2 style="font-size: 16px; font-weight: 600; color: #374151; margin-bottom: 16px; padding-bottom: 8px; border-bottom: 1px solid #e5e7eb;">{{ $__ui('Uploaded Files', 'Fail Dimuat Naik') }}</h2>
            ...
            <div style="background: white; border-radius: 20px; padding: 32px; box-shadow: 0 10px 30px rgba(0, 0, 0, 0.1); border: 1px solid #e2e8f0; margin-bottom: 32px;">
                <h2 style="font-size: 24px; font-weight: 700; color: #1e293b; margin-bottom: 20px;">{{ $__ui('📋 Extracted Content from Files', '📋 Kandungan Diekstrak daripada Fail') }}</h2>
                <div style="background: linear-gradient(135deg, #fef3c7 0%, #fde68a 100%); padding: 24px; border-radius: 16px; border: 1px solid #f59e0b;">
                    <p style="color: #92400e; font-size: 16px; margin-bottom: 16px; font-weight: 600;">{{ $__ui('📊 NUJUM-Processed File Content', '📊 Kandungan Fail Diproses NUJUM') }}</p>
                    ...
                    <p style="color: #92400e; font-size: 14px; margin-top: 16px; margin-bottom: 0; opacity: 0.8;">
                        {{ $__ui('This content was automatically extracted from your uploaded files and analyzed by the NUJUM system to enhance the prediction accuracy.', 'Kandungan ini diekstrak secara automatik daripada fail yang dimuat naik dan dianalisis oleh sistem NUJUM untuk meningkatkan ketepatan ramalan.') }}
                    </p>
                </div>
            </div>
            @endif
            ...
            <div style="margin-bottom: 32px;">
                <h2 style="font-size: 16px; font-weight: 600; color: #374151; margin-bottom: 16px; padding-bottom: 8px; border-bottom: 1px solid #e5e7eb;">{{ $__ui('Source References', 'Rujukan Sumber') }}</h2>
                <div style="display: flex; flex-direction: column; gap: 8px;">
                    ...
                    <a href="{{ $sourceUrl }}" 
                       ...
                       style="display: block; padding: 10px 12px; background: #fef3c7; color: #92400e; text-decoration: none; border-radius: 6px; font-weight: 500; font-size: 13px; transition: all 0.3s ease; border: 1px solid #fde68a; word-wrap: break-word; overflow-wrap: break-word;">
                        {{ $__ui('Source', 'Sumber') }} {{ $index + 1 }}: <span class="source-url-text">{{ Str::limit($sourceUrl, 60) }}</span>
                    </a>
                    @endforeach
                </div>
            </div>
            ...
            <div style="margin-bottom: 32px;">
                <h2 style="font-size: 16px; font-weight: 600; color: #374151; margin-bottom: 16px; padding-bottom: 8px; border-bottom: 1px solid #e5e7eb;">{{ $__ui('Source Content Analysis', 'Analisis Kandungan Sumber') }}</h2>
                <div style="padding: 20px; background: #f8fafc; border-radius: 8px; border: 1px solid #e2e8f0; border-left: 3px solid #10b981;">
                    <p style="color: #374151; font-size: 14px; margin-bottom: 12px; font-weight: 600;">{{ $__ui('Content Extraction Results', 'Keputusan Pengekstrakan Kandungan') }}</p>
                    <div style="color: #374151; font-size: 14px; line-height: 1.6;">
                        <p style="margin-bottom: 8px;"><strong>{{ $__ui('Total Sources:', 'Jumlah Sumber:') }}</strong> {{ $prediction->prediction_result['scraping_metadata']['total_sources'] }}</p>
                        <p style="margin-bottom: 8px;"><strong>{{ $__ui('Successfully Scraped:', 'Berjaya Dikikis:') }}</strong> {{ $prediction->prediction_result['scraping_metadata']['successfully_scraped'] }}</p>
                        <p style="margin-bottom: 12px;"><strong>{{ $__ui('Analysis Date:', 'Tarikh Analisis:') }}</strong> {{ \Carbon\Carbon::parse($prediction->prediction_result['scraping_metadata']['scraped_at'])->format('M d, Y H:i') }}</p>
                        
                        @if(isset($prediction->prediction_result['scraping_metadata']['source_details']))
                        <div style="margin-top: 16px;">
                            <p style="font-weight: 600; margin-bottom: 12px; color: #374151;">{{ $__ui('Source Details:', 'Butiran Sumber:') }}</p>
                            ...
                            <div style="background: white; padding: 12px; border-radius: 6px; border: 1px solid #e2e8f0; margin-bottom: 8px;">
                                <p style="margin: 0 0 4px 0; color: #374151; word-wrap: break-word; overflow-wrap: break-word;"><strong>{{ $__ui('Source', 'Sumber') }} {{ $index + 1 }}:</strong> 
                                ...
                                <p style="margin: 0 0 4px 0; font-size: 13px; color: #64748b;"><strong>{{ $__ui('Title:', 'Tajuk:') }}</strong> {{ $source['title'] }}</p>
                                <p style="margin: 0 0 4px 0; font-size: 13px; color: #64748b;"><strong>{{ $__ui('Word Count:', 'Kiraan Perkataan:') }}</strong> {{ $source['word_count'] }}</p>
                                <p style="margin: 0; font-size: 13px; color: #64748b;"><strong>{{ $__ui('Status:', 'Status:') }}</strong> 
                                    <span style="color: #10b981; font-weight: 600;">
                                        {{ $__ui('Success', 'Berjaya') }}
                                    </span>
                                </p>
                                ...
                                <div style="margin: 8px 0 0 0; padding: 8px; background: #eff6ff; border-radius: 6px; border-left: 3px solid #3b82f6;">
                                    <p style="margin: 0; font-size: 12px; color: #1e40af;">
                                        <strong>{{ $__ui('NUJUM Note:', 'Nota NUJUM:') }}</strong> {{ $__ui('This source was inaccessible, so NUJUM used its general knowledge and training data to provide relevant insights for the analysis.', 'Sumber ini tidak dapat diakses, jadi NUJUM menggunakan pengetahuan am dan data latihan untuk memberikan pandangan berkaitan analisis.') }}
                                    </p>
                                </div>
                                @endif
                            </div>
                    ...
                    <p style="color: #64748b; font-size: 13px; margin-top: 16px; margin-bottom: 0;">
                        {{ $__ui('This analysis incorporates actual content extracted from the provided source URLs, ensuring predictions are based on real, current data rather than general knowledge.', 'Analisis ini menggabungkan kandungan sebenar yang diekstrak daripada URL sumber yang diberikan, memastikan ramalan berdasarkan data nyata dan terkini.') }}
                    </p>
                </div>
            </div>
            @endif
            ...
            <div style="margin-bottom: 32px;">
                <h2 style="font-size: 16px; font-weight: 600; color: #374151; margin-bottom: 16px; padding-bottom: 8px; border-bottom: 1px solid #e5e7eb;">{{ $__ui('Source Analysis & Influence', 'Analisis & Pengaruh Sumber') }}</h2>
                <div style="padding: 20px; background: #f8fafc; border-radius: 8px; border: 1px solid #e2e8f0; border-left: 3px solid #3b82f6;">
                    <p style="color: #374151; font-size: 14px; margin-bottom: 12px; font-weight: 600;">{{ $__ui('How Sources Influenced This Analysis', 'Bagaimana Sumber Mempengaruhi Analisis Ini') }}</p>
                    <div style="color: #374151; font-size: 14px; line-height: 1.6;">
                        {!! nl2br(convertMarkdownBold($prediction->prediction_result['source_analysis'])) !!}
                    </div>
                    <p style="color: #64748b; font-size: 13px; margin-top: 16px; margin-bottom: 0;">
                        {{ $__ui('This analysis shows how each provided source contributed to specific predictions and conclusions, ensuring transparency and traceability of insights.', 'Analisis ini menunjukkan bagaimana setiap sumber menyumbang kepada ramalan dan kesimpulan tertentu, demi ketelusan dan kebolehkesanan pandangan.') }}
                    </p>
                </div>
            </div>
            @endif
            ...
            <div style="margin-bottom: 32px;">
                <h2 style="font-size: 16px; font-weight: 600; color: #374151; margin-bottom: 20px; padding-bottom: 8px; border-bottom: 1px solid #e5e7eb;">{{ $__ui('NUJUM Analysis Results', 'Keputusan Analisis NUJUM') }}</h2>
                ...
                @if(isset($prediction->prediction_result['title']) && is_string($prediction->prediction_result['title']))
                    ...
                    <div style="display: flex; flex-direction: column; gap: 24px;">
                        ...
                        <div style="padding: 20px; background: #f8fafc; border-radius: 8px; border: 1px solid #e2e8f0; border-left: 3px solid #3b82f6;">
                            <h4 style="font-weight: 600; color: #1e293b; margin-bottom: 12px; font-size: 16px;">{{ $__ui('Executive Summary', 'Ringkasan Eksekutif') }}</h4>
                            <div style="color: #374151; line-height: 1.6; font-size: 14px;">
                                @if(is_array($report['executive_summary']))
                                    @foreach($report['executive_summary'] as $key => $value)
                                        @if(is_string($value))
                                            <p style="margin-bottom: 10px;"><strong>{{ $__jsonKeyLabel($key) }}:</strong> {!! convertMarkdownBold($value) !!}</p>
                                        @elseif(is_array($value))
                                            <div style="margin-bottom: 12px;">
                                                <p style="font-weight: 600; margin-bottom: 6px;">{{ $__jsonKeyLabel($key) }}:</p>
                                        ...
                                        @else
                                            <p style="margin-bottom: 10px;"><strong>{{ $__jsonKeyLabel($key) }}:</strong> {!! convertMarkdownBold((string)$value) !!}</p>
                                        @endif
                                    @endforeach
                        ...
                        <div style="padding: 20px; background: #f8fafc; border-radius: 8px; border: 1px solid #e2e8f0; border-left: 3px solid #6b7280;">
                            <h4 style="font-weight: 600; color: #1e293b; margin-bottom: 12px; font-size: 16px;">{{ $__ui('Current Situation & Future Implications', 'Situasi Semasa & Implikasi Masa Hadapan') }}</h4>
                            <div style="color: #374151; line-height: 1.6; font-size: 14px;">
                        ...
                        <div style="padding: 20px; background: #f8fafc; border-radius: 8px; border: 1px solid #e2e8f0; border-left: 3px solid #10b981;">
                            <h4 style="font-weight: 600; color: #1e293b; margin-bottom: 12px; font-size: 16px;">{{ $__ui('Key Factors for Future Development', 'Faktor Utama untuk Pembangunan Masa Hadapan') }}</h4>
                        ...
                        <div style="padding: 20px; background: #f8fafc; border-radius: 8px; border: 1px solid #e2e8f0; border-left: 3px solid #8b5cf6;">
                            <h4 style="font-weight: 600; color: #1e293b; margin-bottom: 12px; font-size: 16px;">{{ $__ui('Future Predictions', 'Ramalan Masa Hadapan') }}</h4>
                            <ul style="margin: 0; padding-left: 18px; color: #374151; line-height: 1.6; font-size: 14px; list-style: none;">
                        ...
                        <div style="padding: 20px; background: #f8fafc; border-radius: 8px; border: 1px solid #e2e8f0; border-left: 3px solid #ef4444;">
                            <h4 style="font-weight: 600; color: #1e293b; margin-bottom: 12px; font-size: 16px;">{{ $__ui('Risk Assessment', 'Penilaian Risiko') }}</h4>
                            <div style="color: #374151; line-height: 1.6; font-size: 14px;">
                                    ...
                                    @if($isArrayOfObjects)
                                        <div class="risk-table-container" style="overflow-x: auto; -webkit-overflow-scrolling: touch;">
                                            <table class="risk-table" style="width: 100%; border-collapse: collapse; margin: 12px 0; font-size: 14px; min-width: 600px;">
                                                <thead>
                                                    <tr>
                                                        <th style="background-color: #f0f0f0; color: #1e293b; font-weight: 600; padding: 10px; text-align: left; border: 1px solid #d1d5db; width: 35%;">{{ $__ui('Risk', 'Risiko') }}</th>
                                                        <th style="background-color: #f0f0f0; color: #1e293b; font-weight: 600; padding: 10px; text-align: left; border: 1px solid #d1d5db; width: 15%;">{{ $__ui('Level', 'Tahap') }}</th>
                                                        <th style="background-color: #f0f0f0; color: #1e293b; font-weight: 600; padding: 10px; text-align: left; border: 1px solid #d1d5db; width: 15%;">{{ $__ui('Probability', 'Kebarangkalian') }}</th>
                                                        <th style="background-color: #f0f0f0; color: #1e293b; font-weight: 600; padding: 10px; text-align: left; border: 1px solid #d1d5db; width: 35%;">{{ $__ui('Mitigation Strategy', 'Strategi Pengurangan') }}</th>
                                                    </tr>
                                                </thead>
                                                <tbody>
                                                    @foreach($report['risk_assessment'] as $risk)
                                                        @if(is_array($risk))
                                                            <tr style="background-color: {{ $loop->even ? '#f9fafb' : 'white' }};">
                                                                <td style="padding: 10px; border: 1px solid #e5e7eb; vertical-align: top; word-wrap: break-word; overflow-wrap: break-word;">{!! isset($risk['risk']) ? (is_string($risk['risk']) ? convertMarkdownBold($risk['risk']) : convertMarkdownBold((string)$risk['risk'])) : $__ui('N/A', 'T/B') !!}</td>
                                                                <td style="padding: 10px; border: 1px solid #e5e7eb; vertical-align: top; word-wrap: break-word;">{!! isset($risk['level']) ? convertMarkdownBold($__riskScaleLabel($risk['level'])) : '-' !!}</td>
                                                                <td style="padding: 10px; border: 1px solid #e5e7eb; vertical-align: top; word-wrap: break-word;">{!! isset($risk['probability']) ? convertMarkdownBold($__riskScaleLabel($risk['probability'])) : '-' !!}</td>
                        ...
                        <div style="padding: 20px; background: #f8fafc; border-radius: 8px; border: 1px solid #e2e8f0; border-left: 3px solid #f59e0b;">
                            <h4 style="font-weight: 600; color: #1e293b; margin-bottom: 12px; font-size: 16px;">{{ $__ui('Policy Implications', 'Implikasi Dasar') }}</h4>
                            <div style="color: #374151; line-height: 1.6; font-size: 14px;">
                        ...
                        <div style="padding: 20px; background: #f8fafc; border-radius: 8px; border: 1px solid #e2e8f0; border-left: 3px solid #0ea5e9;">
                            <h4 style="font-weight: 600; color: #1e293b; margin-bottom: 12px; font-size: 16px;">{{ $__ui('Strategic Recommendations', 'Cadangan Strategik') }}</h4>
                            <ul style="margin: 0; padding-left: 18px; color: #374151; line-height: 1.6; font-size: 14px; list-style: none;">
                        ...
                        <div style="padding: 20px; background: #f8fafc; border-radius: 8px; border: 1px solid #e2e8f0; border-left: 3px solid #10b981;">
                            <h4 style="font-weight: 600; color: #1e293b; margin-bottom: 12px; font-size: 16px;">{{ $__ui('Prediction Confidence', 'Keyakinan Ramalan') }}</h4>
                        ...
                        <div style="padding: 20px; background: #f8fafc; border-radius: 8px; border: 1px solid #e2e8f0; border-left: 3px solid #fbbf24;">
                            <h4 style="font-weight: 600; color: #1e293b; margin-bottom: 12px; font-size: 16px;">{{ $__ui('Strategic Implications', 'Implikasi Strategik') }}</h4>
                            <ul style="margin: 0; padding-left: 18px; color: #374151; line-height: 1.6; font-size: 14px; list-style: none;">
                        ...
                        <div style="padding: 20px; background: #f8fafc; border-radius: 8px; border: 1px solid #e2e8f0; border-left: 3px solid #3b82f6;">
                            <h4 style="font-weight: 600; color: #1e293b; margin-bottom: 12px; font-size: 16px;">{{ $__ui('Data Sources & Methodology', 'Sumber Data & Metodologi') }}</h4>
                        ...
                        <div style="padding: 20px; background: #f8fafc; border-radius: 8px; border: 1px solid #e2e8f0; border-left: 3px solid #a855f7;">
                            <h4 style="font-weight: 600; color: #1e293b; margin-bottom: 12px; font-size: 16px;">{{ $__ui('Key Assumptions', 'Andaian Utama') }}</h4>
                            <ul style="margin: 0; padding-left: 18px; color: #374151; line-height: 1.6; font-size: 14px;">
                        ...
                        <div style="padding: 20px; background: #f8fafc; border-radius: 8px; border: 1px solid #e2e8f0; border-left: 3px solid #10b981;">
                            <h4 style="font-weight: 600; color: #1e293b; margin-bottom: 12px; font-size: 16px;">{{ $__ui('Success Metrics & KPIs', 'Metrik Kejayaan & KPI') }}</h4>
                            <ul style="margin: 0; padding-left: 18px; color: #374151; line-height: 1.6; font-size: 14px;">
                        ...
                        <div style="padding: 20px; background: #f8fafc; border-radius: 8px; border: 1px solid #e2e8f0; border-left: 3px solid #f59e0b;">
                            <h4 style="font-weight: 600; color: #1e293b; margin-bottom: 12px; font-size: 16px;">{{ $__ui('Timeline & Review Schedule', 'Garis Masa & Jadual Semakan') }}</h4>
                            <div style="color: #374151; line-height: 1.6; font-size: 14px;">
                                @if(isset($report['critical_timeline']) && is_string($report['critical_timeline']))
                                    <p style="margin-bottom: 8px;"><strong>{{ $__ui('Critical Timeline:', 'Garis Masa Kritikal:') }}</strong> {{ $report['critical_timeline'] }}</p>
                                @endif
                                @if(isset($report['next_review']) && is_string($report['next_review']))
                                    <p style="margin-bottom: 8px;"><strong>{{ $__ui('Next Review Date:', 'Tarikh Semakan Seterusnya:') }}</strong> {{ $report['next_review'] }}</p>
                                @endif
                                @if(isset($report['analysis_date']) && is_string($report['analysis_date']))
                                    <p style="margin: 0;"><strong>{{ $__ui('Analysis Date:', 'Tarikh Analisis:') }}</strong> {{ $report['analysis_date'] }}</p>
                                @endif
                            </div>
                        </div>
                        @endif
                        ...
                        <div style="padding: 20px; background: #f8fafc; border-radius: 8px; border: 1px solid #e2e8f0;">
                            <h4 style="font-weight: 600; color: #1e293b; margin-bottom: 12px; font-size: 16px;">{{ $__ui('Analysis Results', 'Keputusan Analisis') }}</h4>
                ...
                @elseif(isset($prediction->prediction_result['raw_response']))
                    ...
                    <div style="padding: 20px; background: #fef3c7; border: 1px solid #f59e0b; color: #92400e; border-radius: 8px; margin-bottom: 20px; border-left: 4px solid #f59e0b;">
                        <div style="margin-bottom: 12px;">
                            <strong style="font-size: 14px;">{{ $__ui('Partial Analysis Results', 'Keputusan Analisis Separuh') }}</strong>
                        </div>
                        <p style="margin-bottom: 16px; font-size: 13px;">{{ $__ui('The NUJUM analysis was completed but the response may be incomplete. Below is the raw output from NUJUM:', 'Analisis NUJUM telah selesai tetapi respons mungkin tidak lengkap. Di bawah adalah output mentah daripada NUJUM:') }}</p>
                    </div>
                    
                    <div style="padding: 20px; background: #f8fafc; border-radius: 8px; border: 1px solid #e2e8f0;">
                        <h4 style="font-weight: 600; color: #1e293b; margin-bottom: 12px; font-size: 16px;">{{ $__ui('NUJUM Analysis Output', 'Output Analisis NUJUM') }}</h4>
            ...
            <div style="margin-bottom: 32px; text-align: center; padding: 40px 20px; background: #fee2e2; border-radius: 8px; border: 1px solid #fca5a5;">
                <h3 style="font-size: 18px; font-weight: 600; color: #991b1b; margin-bottom: 12px;">{{ $__ui('Analysis Failed', 'Analisis Gagal') }}</h3>
                <p style="color: #64748b; line-height: 1.6; margin: 0; font-size: 14px;">{{ $__ui('The NUJUM prediction analysis could not be completed. Please try again or contact support.', 'Analisis ramalan NUJUM tidak dapat diselesaikan. Sila cuba lagi atau hubungi sokongan.') }}</p>
            </div>
            @elseif($prediction->status === 'processing')
            <div style="margin-bottom: 32px; text-align: center; padding: 40px 20px; background: #fef3c7; border-radius: 8px; border: 1px solid #fde68a;">
                <h3 style="font-size: 18px; font-weight: 600; color: #92400e; margin-bottom: 12px;">{{ $__ui('Processing...', 'Memproses...') }}</h3>
                <p style="color: #64748b; line-height: 1.6; margin: 0; font-size: 14px;">{{ $__ui('Your prediction analysis is being processed by NUJUM. This may take a few moments.', 'Analisis ramalan anda sedang diproses oleh NUJUM. Ini mungkin mengambil masa beberapa ketika.') }}</p>
            </div>
            @endif
