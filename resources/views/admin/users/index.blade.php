        ...
        <div class="card-header bg-white border-0 py-3">
            <div class="d-flex flex-column flex-sm-row justify-content-between align-items-start align-items-sm-center">
                <h5 class="card-title mb-0 fw-semibold">
                    <i class="bi bi-person-badge text-primary me-2"></i>Client Accounts
                </h5>
                            ...
                            <tr data-user-id="{{ $user->id }}" data-status="{{ $user->last_login_at && ($user->last_login_at instanceof \Carbon\Carbon || is_string($user->last_login_at)) ? 'active' : 'inactive' }}" data-activity="{{ $user->predictions_count > 0 ? 'with_predictions' : 'no_predictions' }}">
                                ...
                                <td class="px-3 py-3 d-none d-md-table-cell">
                                    <span class="badge bg-info rounded-pill">
                                        <i class="bi bi-graph-up me-1"></i>{{ \App\Models\Prediction::where('user_id', $user->id)->count() }}
                                    </span>
                                </td>
                                ...
                                <td class="px-3 py-3">
                                    <div class="btn-group" role="group">
                                        <button class="btn btn-outline-primary btn-sm" onclick="viewClientDetails({{ $user->id }})" title="View Details">
                                            <i class="bi bi-eye"></i>
                                        </button>
