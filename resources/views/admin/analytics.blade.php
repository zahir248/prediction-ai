...
<div class="container-fluid">
    ...
    <div class="d-flex flex-column flex-sm-row justify-content-between align-items-start align-items-sm-center mb-4">
        <div class="mb-3 mb-sm-0">
            ...
            <p class="text-muted mb-0">Track usage, costs, and performance metrics for all AI analyses with comprehensive insights.</p>
        </div>
    </div>
                        ...
                        <div class="table-responsive">
                            <table class="table table-hover align-middle mb-0">
                                ...
                                <tbody>
                                        ...
                                        <tr>
                                            <td class="px-4 py-3">
                                                <div class="d-flex align-items-center">
                                                    ...
                                                    <div>
                                                        <h6 class="mb-1 fw-semibold text-dark">{{ $user->user->name ?? 'Unknown' }}</h6>
                                                        <small class="text-muted d-block">{{ $user->user->email ?? 'N/A' }}</small>
                                                    </div>
                                                </div>
                                            </td>
                                            <td class="px-4 py-3">
                                                <span class="badge bg-primary rounded-pill px-3">{{ number_format($user->analysis_count) }}</span>
                                            </td>
                                            <td class="px-4 py-3">
                                                <span class="badge bg-success rounded-pill px-3">{{ number_format($user->total_tokens) }}</span>
                                            </td>
                                            <td class="px-4 py-3">
                                                <div class="progress" style="height: 24px;">
                                                    <div class="progress-bar bg-info" role="progressbar" 
                                                         style="width: {{ $analytics['total_tokens'] > 0 ? ($user->total_tokens / $analytics['total_tokens']) * 100 : 0 }}%">
                                                        <span class="fw-semibold">{{ $analytics['total_tokens'] > 0 ? number_format(($user->total_tokens / $analytics['total_tokens']) * 100, 1) : '0' }}%</span>
                                                    </div>
                                                </div>
                                            </td>
                                        </tr>
                                    ...
                                    @else
                                        <tr>
                                            <td colspan="4" class="text-center px-4 py-5">
            ...
            <div class="card border-0 shadow-sm h-100">
                <div class="card-header bg-white border-0 py-3">
                    <h5 class="card-title mb-0 fw-semibold d-flex align-items-center">
                        ...
                        <span class="text-dark">Daily Usage Trend</span>
                    </h5>
                </div>
                <div class="card-body">
                    <div class="chart-container" style="position: relative; height:300px;">
                        <div class="d-flex align-items-end justify-content-between" style="height: 250px;">
                            @if(!empty($analytics['daily_usage_trend']))
                                @foreach($analytics['daily_usage_trend'] as $date => $data)
                                <div class="d-flex flex-column align-items-center">
                                    <div class="rounded-top bg-primary bg-opacity-75" style="width: 32px; height: {{ $data['analysis_count'] > 0 ? ($data['analysis_count'] / max(array_column($analytics['daily_usage_trend'], 'analysis_count'))) * 200 : 0 }}px;"></div>
                                    <small class="text-muted mt-2 fw-semibold">{{ \Carbon\Carbon::parse($date)->format('M d') }}</small>
                                    <small class="text-primary fw-semibold">{{ $data['analysis_count'] }}</small>
                                </div>
                                @endforeach
                            @else
                    ...
                    <thead class="table-light">
                        <tr>
                            ...
                            <tbody>
                                ...
                                @else
                                    <tr>
                                        <td colspan="22" class="text-center" style="color: #9ca3af;">
...
<style>
/* Additional responsive styles for admin analytics */
...
@media (max-width: 768px) {
    ...
    .search-filter-row .col-12:last-child {
        margin-bottom: 0;
    }
